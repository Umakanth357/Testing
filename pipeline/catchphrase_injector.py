...
def inject_debate_catchphrases(script: str) -> str:
    """
    For debate format: ensure NAVYA lines start with her opener and ARJUN's too.
    Works on the tagged NAVYA: / ARJUN: format.
    """
    lines    = script.strip().splitlines()
    navya_c  = get_character("navya")
    ...
    for line in lines:
        stripped = line.strip()
        if stripped.upper().startswith("NAVYA:"):
            navya_lines_seen += 1
            if navya_lines_seen == 1:
                # First NAVYA line — ensure opener
                content = stripped[6:].strip()
                has_opener = any(w in content.lower() for w in ["namaskaram", "ayyo", "hello"])
                if not has_opener:
                    recently = _get_used_recently("navya", "opener")
                    opener   = _pick_fresh(navya_c.opener, recently)
                    output.append(f"NAVYA: {opener}")
                    _log_used("navya", "opener", opener)
    ...
    if "navya" in last_block and not any(w in last_block for w in ["next week", "bye", "namaskaram"]):
        recently = _get_used_recently("navya", "sign_off")
        sign_off = _pick_fresh(navya_c.sign_off, recently)
        output.append(f"NAVYA: {sign_off}")
        _log_used("navya", "sign_off", sign_off)
