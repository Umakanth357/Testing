...
def get_prediction_scoreboard() -> dict:
    ...
    return {
        "navya": {
            "total":    stats.get("navya", {}).get("total_predictions", 0),
            "correct":  stats.get("navya", {}).get("correct_predictions", 0),
            "accuracy": stats.get("navya", {}).get("accuracy_pct", 0.0),
        },
        "arjun": {
...
def format_scoreboard_text(scoreboard: dict) -> str:
    ...
    a = scoreboard["arjun"]
    return (
        f"Navya: {k['accuracy']}% ({k['correct']}/{k['total']})  |  "
